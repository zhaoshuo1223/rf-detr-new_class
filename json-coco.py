...
class labelme2coco(object):
    ...
    def _find_image_in_dir(self, directory, stem_or_name):
        """在指定目录中按文件名或stem查找图片，优先精确文件名。"""
        if not directory or not os.path.isdir(directory) or not stem_or_name:
            return ""
        # 先按完整文件名查找
        exact = os.path.join(directory, os.path.basename(stem_or_name))
        if os.path.exists(exact):
            return exact
        # 再按 stem + 常见后缀查找
        stem = os.path.splitext(os.path.basename(stem_or_name))[0]
        for ext in IMAGE_EXTENSIONS + [e.upper() for e in IMAGE_EXTENSIONS]:
            p = os.path.join(directory, stem + ext)
            if os.path.exists(p):
                return p
        return ""

    def _resolve_image_path(self, json_file, data, image_info):
        """解析图片路径：优先从全局 IMAGE_DIR 按 json 对应名称查找。"""
        json_dir = os.path.dirname(json_file)
        image_path_raw = data.get("imagePath", "")
        file_name = image_info.get("file_name", "")
        json_base = os.path.splitext(os.path.basename(json_file))[0]

        # 1) 优先在 IMAGE_DIR 中按 json同名 / imagePath名 查找
        p = self._find_image_in_dir(IMAGE_DIR, json_base)
        if p:
            return p
        p = self._find_image_in_dir(IMAGE_DIR, file_name or image_path_raw)
        if p:
            return p

        # 2) 再尝试 imagePath 原始路径（绝对路径直接用，相对路径相对 json_dir）
        if image_path_raw:
            if os.path.isabs(image_path_raw) and os.path.exists(image_path_raw):
                return image_path_raw
            p = os.path.join(json_dir, image_path_raw)
            if os.path.exists(p):
                return p

        # 3) 最后回退到 json 同目录查找
        p = self._find_image_in_dir(json_dir, file_name or json_base)
        if p:
            return p
        return ""
    ...
    def data_transfer(self):
        ...
        for num, json_file in enumerate(self.labelme_json):
            with open(json_file, "r", encoding="utf-8") as fp:
                ...
                # 保存原始图片路径（优先在 IMAGE_DIR 按 json对应名称查找）
                image_path = self._resolve_image_path(json_file, data, image_info)
